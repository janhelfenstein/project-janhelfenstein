...
|                                      |                                       |
|------------------------------------|------------------------------------|
| ![](images/clipboard-2887181069.png) | ![](images/clipboard-3782636616.jpeg) |
...
The second figure @fig-weekday displays the number of answers per day, while also showing a more visible distinction between male and female responses. 11 people were met in-person in the forest on Sunday evening, of which most were male. From Monday to Thursday, responses came from people answering the questionnaire online. Most answers came in on Monday, when the link to the survey was posted in a WhatsApp group chat for students living in student housing on Campus Hoenggerberg, which is only 200 meters away from the Kaeferberg forest. Most of these participants were also male.
...
# Plot using geom_col and add text labels
ggplot(data = survey_demography, 
       aes(x = weekday, y = amount, fill = gender)) + 
...
survey_vis2 <- survey_small |> 
  group_by(weekday) |> 
  mutate(amount = n())

ggplot(data = survey_vis2, 
  ...
  geom_bar() +
  geom_text(amount) +
  labs(x = "weekday the survey was taken", y = "number of answers")
```
