...
date: 2025-06-05
author:
  - name: Jan Helfenstein
    orcid: 0000-0003-2196-5015
    email: janh@student.ethz.ch
    affiliation: 
      - name: ETH Zurich
        url: https://ethz.ch/de.html

abstract: 
  A study on waste in forests around Zuerich and its perception by forest users in June 2025.
license: "CC BY"
citation: 
  container-title: My GitHub
  url: https://github.com/janhelfenstein
...
ggplot(data = survey_small, 
  ...
  geom_point(size = 3) + 
  labs(title = "Survey timeline and perceived cleanliness of the forest",
       x = "survey timestamp", y = "perceived forest cleanliness") +
...
The same conclusions can be made from the following graphic @fig-age-gender-rating , which shows yet again how participants rate the cleanliness of Zurichs forests, charted by age and gender.
...
ggplot(data = survey_small,
  ...
  geom_point(size = 3) +
  labs(title = "Relation between age, gender and perceived forest cleanliness",
       x = "age in years", 
...
survey_table_gender2 |> 
  gt() |> 
  fmt_number(decimals = 1) |> 
  tab_header(title = "Participants visiting frequency, their value of forest sports, and their suggested waste-removal days per year",
             subtitle = "Data from 27 forest users in June 2025, grouped by gender")
```
...
#| fig-cap: "Suggested number of days per year for waste-removal by authorities"
ggplot(data = survey_small, 
       mapping = aes(x = age,
  ...
  geom_point(size = 3) +
  labs(title = "Suggested number of days per year for waste-removal by authorities",
       x = "age in years", 
...
```{r}
waste_types <- survey2 |> 
  select(id, waste_type_today) |> 
  group_by(waste_type_today) |> 
  distinct() |> 
  summarize(count = n())

write_csv(waste_types, here::here("data/final/waste_types.csv"))
```

The following @fig-waste-types shows the different trash items that forest users found on that day. Not much more can be interpreted from this data, other than there seems to be a trend to consumer items such as plastic bottles, cigarettes, cans and food packaging landing in the forest more often. However, almost nobody used the "other" option in the survey to manually report other trash items found in the forest. In fact, only 1 person used this option, and wrote "horse shit".

```{r}
#| label: fig-waste-types
#| fig-cap: "Waste types seen by forest users"

ggplot(data = waste_types,
       mapping = aes(x = waste_type_today,
                     y = count)
) +
  geom_col() +
  labs(x = "Waste type seen today")

```

```{r}
waste_location <- survey2 |> 
  select(id, waste_location) |> 
  group_by(waste_location) |> 
  distinct() |> 
  select(!id) |> 
  summarize(count = n())

waste_location_today <- survey2 |> 
  select(id, waste_location_today) |>
  rename(waste_location = waste_location_today) |> 
  group_by(waste_location) |> 
  distinct() |> 
  select(!id) |> 
  summarize(count = n())

write_csv(waste_location, here::here("data/final/waste_location.csv"))
write_csv(waste_location_today, here::here("data/final/waste_location_today.csv"))
```

The following graph @fig-waste-location displays how forest users report where they see waste in the forest in general, and on the day they took the survey. What stands out is that people think that waste is concentrated mainly around picnic areas. But on the other hand, on the day they took the survey, they saw most of the waste along paths or trails. There might be some bias, coming from past experiences where people saw very dirty picnic areas in the forest.

```{r}
#| label: fig-waste-location
#| fig-cap: "Comparison of waste locations in the forest, in general and today"

# I used ChatGPT to combine the 2 dataframes, prompt:
# https://chatgpt.com/share/68421141-9c14-8011-9448-3ecde5165ada
waste_location$source <- "forest waste in general"
waste_location_today$source <- "forest waste today"

waste_combined <- rbind(waste_location, waste_location_today)
write_csv(waste_combined, here::here("data/final/waste_combined.csv"))

ggplot(data = waste_combined, 
       aes(x = waste_location, 
           y = count, 
           fill = source)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Comparison of waste locations in the forest, in general and today",
       x = "location", 
       y = "count") +
  theme_minimal()
```
